...
var IScaler = {
  basePath:appPath + "/uploads/imgcompare/"
...
module.exports = {
  ...
  compare:function(req,res){
    var inputImgA = IScaler.basePath+"1.png";
    var inputImgB = IScaler.basePath+"2.png";
    fs.createReadStream(inputImgA)
      .pipe(new PNG({
          filterType: 4
      }))
      .on('parsed', function() {
          var thisA = this;
          fs.createReadStream(inputImgB)
            .pipe(new PNG({
                filterType: 4
            }))
            .on('parsed', function() {
              var thisB = this;
              for (var yA = 0; yA < thisA.height; yA++) {
                for (var xA = 0; xA < thisA.width; xA++) {
                  var idx = (thisA.width * yA + xA) << 2;
                  if(thisA.data[idx] === thisB.data[idx] &&
                    thisA.data[idx+1] === thisB.data[idx+1] &&
                    thisA.data[idx+2] === thisB.data[idx+2] &&
                    thisA.data[idx+3] === thisB.data[idx+3]
                  ){
              ...
              thisA.pack().pipe(fs.createWriteStream(IScaler.basePath+"x.png"));
              res.send("done");
            });
      });
  }
